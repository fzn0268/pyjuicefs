...
def test_rename(jfs, jfs_specfs, filename, filename2, dirname):
    jfs_specfs.mkdir(dirname)
    jfs.create(filename)
    with pytest.raises(FileExistsError):
        jfs_specfs.rename(filename, dirname)

    assert jfs_specfs.exists(filename2) is False
    assert jfs_specfs.exists(filename) is True
    jfs_specfs.rename(filename, filename2)
    assert jfs_specfs.exists(filename2) is True
    assert jfs_specfs.exists(filename) is False
...
def test_replace(jfs, jfs_specfs, filename, filename2, dirname):
    test_rename(jfs, jfs_specfs, filename, filename2, dirname)
...
def test_makedirs(jfs_specfs, dirname, dirname2, dirname3):
    assert jfs_specfs.isdir(dirname) is False
    jfs_specfs.makedirs(dirname)

    assert jfs_specfs.isdir(dirname) is True
    jfs_specfs.makedirs(dirname, exist_ok=True)

    with pytest.raises(OSError):
        jfs_specfs.makedirs(dirname)

    assert jfs_specfs.isdir(dirname2) is False
    jfs_specfs.makedirs(dirname2)

    assert jfs_specfs.isdir(dirname2) is True
    jfs_specfs.makedirs(dirname2, exist_ok=True)

    with pytest.raises(OSError):
        jfs_specfs.makedirs(dirname2)

    assert jfs_specfs.isdir(dirname3) is False
    jfs_specfs.makedirs(dirname3)

    assert jfs_specfs.isdir(dirname3) is True
    jfs_specfs.makedirs(dirname3, exist_ok=True)
...
def test_mkdir(jfs_specfs, dirname):
    jfs_specfs.mkdir(dirname)
    assert jfs_specfs.exists(dirname) is True
...
def test_rmdir(jfs, jfs_specfs, filename, dirname):
    jfs_specfs.mkdir(dirname)
    assert jfs_specfs.exists(dirname) is True
    jfs_specfs.rmdir(dirname)
    assert jfs_specfs.exists(dirname) is False

    jfs.create(filename)
    with pytest.raises(NotADirectoryError):
        jfs_specfs.rmdir(filename)
...
def test_exists(jfs, jfs_specfs, filename, dirname):
    assert jfs_specfs.exists(filename) is False
    jfs.create(filename)
    assert jfs_specfs.exists(filename) is True
...
def test_lexists(jfs, jfs_specfs, filename, filename2, filename3, dirname):
    assert jfs_specfs.lexists(filename) is False
    jfs.create(filename)
    assert jfs_specfs.lexists(filename) is True
    jfs.symlink(filename, filename2)
    assert jfs_specfs.lexists(filename2) is True
    jfs.unlink(filename)
    assert jfs_specfs.lexists(filename2) is True
    jfs.unlink(filename2)
    assert jfs_specfs.lexists(filename2) is False

    assert jfs_specfs.lexists(dirname) is False
    jfs_specfs.mkdir(dirname)
    assert jfs_specfs.lexists(dirname) is True
    jfs.symlink(dirname, filename3)
    assert jfs_specfs.lexists(filename3) is True
    jfs_specfs.rmdir(dirname)
    assert jfs_specfs.lexists(filename3) is True
    jfs.unlink(filename3)
    assert jfs_specfs.lexists(filename3) is False
...
def test_listdir(jfs, jfs_specfs, filename, filename2, dirname):
    jfs.create(filename)
    jfs.symlink(filename, filename2)
    jfs_specfs.mkdir(dirname)

    names = list(e['name'][1:] for e in jfs_specfs.listdir("/"))
    assert len(names) == 3
...
def test_walk(jfs, jfs_specfs, dirname):
    jfs_specfs.mkdir(dirname)
    jfs.create(Path(os.path.join(dirname, "file")).as_posix())
    jfs_specfs.mkdir(Path(os.path.join(dirname, "dir")).as_posix())
    jfs.symlink(
        Path(os.path.join(dirname, "file")).as_posix(),
        Path(os.path.join(dirname, "dir", "link")).as_posix(),
    )

    assert list(jfs_specfs.walk(dirname)) == [
        (dirname, ["dir"], ["file"]),
        (Path(os.path.join(dirname, "dir")).as_posix(), [], ["link"]),
    ]

    assert list(jfs_specfs.walk(dirname, topdown=False)) == [
        (Path(os.path.join(dirname, "dir")).as_posix(), [], ["link"]),
        (dirname, ["dir"], ["file"]),
    ]


def test_isfile(jfs, jfs_specfs, filename, filename2, dirname):
    assert jfs_specfs.isfile(filename) is False
    assert jfs_specfs.isfile(filename2) is False
    assert jfs_specfs.isfile(dirname) is False

    jfs.symlink(filename, filename2)
    assert jfs_specfs.isfile(filename2) is False

    jfs.create(filename)
    assert jfs_specfs.isfile(filename) is True
    assert jfs_specfs.isfile(filename2) is True

    jfs_specfs.mkdir(dirname)
    assert jfs_specfs.isfile(dirname) is False


def test_isdir(jfs, jfs_specfs, filename, filename2, dirname):
    assert jfs_specfs.isdir(dirname) is False
    assert jfs_specfs.isdir(filename) is False
    assert jfs_specfs.isdir(filename2) is False
    ...
    assert jfs_specfs.isdir(dirname) is True
    assert jfs_specfs.isdir(dirname + "/") is True
    assert jfs_specfs.isdir(filename2) is True
...
def test_getmtime(jfs, jfs_specfs, filename):
    start = int(time.time())
    jfs.create(filename)
    stop = int(time.time())
    assert start <= int(jfs_specfs.modified(filename).timestamp()) <= stop
...
def test_getsize(jfs, jfs_specfs, filename):
    jfs.create(filename)
    fd = jfs.open(filename, os.O_WRONLY)
    jfs.write(fd, CONTENT)
    jfs.close(fd)
    assert jfs_specfs.size(filename) == len(CONTENT)

    fd = jfs.open(filename, os.O_WRONLY)
    jfs.write(fd, CONTENT2)
    jfs.close(fd)
    assert jfs_specfs.size(filename) == len(CONTENT2)
